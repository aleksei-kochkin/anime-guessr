...
import { FilterConfig, DynamicFilterOption } from '@/lib/strategies/ContentStrategy';
...
interface UniversalFilterPanelProps {
  ...
  dynamicOptionsLoader?: (filterId: string) => Promise<DynamicFilterOption[]>;
}
...
export default function UniversalFilterPanel({
  ...
  const [dynamicOptions, setDynamicOptions] = useState<Record<string, DynamicFilterOption[]>>({});
  ...
  const handleButtonMultiToggle = (filterId: string, value: string) => {
    const currentValues = (localFilters[filterId] as string[]) || [];
  ...
  const handleButtonSingleToggle = (filterId: string, value: number | string) => {
    const currentValue = (localFilters[filterId] as (number | string)[] | undefined)?.[0];
    setLocalFilters({
      ...localFilters,
    ...
    switch (config.type) {
            ...
            <div className="flex flex-wrap gap-2">
              {config.options?.map(option => (
                <Button
                  ...
                  variant={(localFilters[config.id] as string[] | undefined)?.includes(String(option.value)) ? 'filter-active' : 'filter'}
                  size="md"
                >
                  {option.label}
                </Button>
          ...
          <div key={config.id}>
            ...
            <div className="flex flex-wrap gap-2">
              {options.map(option => (
                <Button
                  key={option.id}
                  onClick={() => handleButtonSingleToggle(config.id, option.id)}
                  variant={(localFilters[config.id] as (number | string)[] | undefined)?.includes(option.id) ? 'filter-active' : 'filter'}
                  size="md"
                >
                  {option.label}
                </Button>
      ...
      case 'slider':
        return (
          <div key={config.id}>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              {config.label}: {(localFilters[config.id] as number | undefined) || config.min || 0} {localFilters[config.id] === undefined && '(not set)'}
            </label>
            <div className="relative">
              <input
                ...
                max={config.max || 10}
                step={config.step || 1}
                value={(localFilters[config.id] as number | undefined) || config.min || 0}
                ...
                style={{
                  background: `linear-gradient(to right, rgb(37, 99, 235) 0%, rgb(37, 99, 235) ${(((localFilters[config.id] as number | undefined) || config.min || 0) / (config.max || 10)) * 100}%, rgb(229, 231, 235) ${(((localFilters[config.id] as number | undefined) || config.min || 0) / (config.max || 10)) * 100}%, rgb(229, 231, 235) 100%)`
                }}
              />
            </div>
          </div>
      ...
      case 'text':
        return (
          <div key={config.id}>
            ...
            <input
              type="text"
              value={(localFilters[config.id] as string | undefined) || ''}
        ...
        return (
          <div key={config.id}>
            ...
            <div className="flex gap-2">
              <input
                type="number"
                value={(localFilters[`${config.id}From`] as number | undefined) || ''}
              ...
              <input
                type="number"
                value={(localFilters[`${config.id}To`] as number | undefined) || ''}
    ...
    <div className="relative">
      ...
      {isOpen && (
        <>
          ...
          />
          <div className="absolute right-0 mt-2 w-[calc(100vw-1rem)] sm:w-96 bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 z-50 max-h-[80vh] flex flex-col">
            {/* Header - Fixed */}
            <div className="p-3 sm:p-4 border-b border-gray-200 dark:border-gray-700">
              ...
              {loadError && (
                <div className="mt-3 px-3 py-2 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-900/40 rounded-lg">
